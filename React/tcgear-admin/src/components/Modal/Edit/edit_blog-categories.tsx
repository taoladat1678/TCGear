...
const EditBlogCategory: React.FC<EditBlogCategoryProps> = ({ isOpen, category, onClose, onSave }) => {
    ...
    <div className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center p-4">
      <div className="bg-secondary rounded-lg shadow-lg max-w-md w-full max-h-[90vh] overflow-y-auto relative">
        <div className="flex justify-between items-center p-6 border-b border-primary/20">
          <h3 className="text-lg font-semibold text-accent font-orbitron">Sửa Danh Mục Blog</h3>
          <button
            type="button"
              ...
              <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-primary/20 border-dashed rounded-md">
                <div className="space-y-1 text-center">
                  <div className="flex text-sm text-accent/70 font-open-sans">
                    <label htmlFor="edit-blog-category-file-upload" className="relative cursor-pointer bg-secondary rounded-md font-medium text-primary hover:text-primary/80 focus-within:outline-none">
                      <span>Tải lên tệp</span>
                      <input
                        id="edit-blog-category-file-upload"
                        name="file-upload"
                        type="file"
                        className="sr-only"
                        accept="image/*"
