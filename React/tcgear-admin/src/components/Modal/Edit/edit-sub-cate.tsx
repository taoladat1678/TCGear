...
interface EditSubCateProps {
  subCate: SubCategory;
  onClose: () => void;
  onSuccess: (updatedSubCate: SubCategory) => void;
}

export const EditSubCate: React.FC<EditSubCateProps> = ({ subCate, onClose, onSuccess }) => {
  const [code, setCode] = useState(subCate.code);
  const [name, setName] = useState(subCate.name);
  const [parent, setParent] = useState(subCate.parent);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedSubCate = { ...subCate, code: code.trim(), name: name.trim(), parent, image: file ? URL.createObjectURL(file) : subCate.image };
    onSuccess(updatedSubCate);
  };
    ...
    <div className="modal show" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h3 className="text-lg font-semibold text-accent font-orbitron">Sửa Danh Mục Phụ</h3>
        ...
        <form onSubmit={handleSubmit} className="modal-body">
          {subCate.image && (
            <div id="current-image-container">
              ...
              <img src={subCate.image} alt="Hình ảnh hiện tại" className="w-full h-32 object-cover rounded mb-4" />
            </div>
          )}
